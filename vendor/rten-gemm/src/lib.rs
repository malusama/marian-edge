...
impl<LhsT: GemmInT, RhsT: GemmInT, OutT: GemmOutT> GemmExecutor<LhsT, RhsT, OutT> {
    ...
    /// Restores a matrix previously produced by `prepack_b` for this exact
    /// kernel and matrix shape.
    pub fn restore_packed_b(
        &self,
        data: Vec<u32>,
        rows: usize,
        cols: usize,
        ...
        prepack::restore_packed_b(&*self.kernel, data, rows, cols, packed_kernel_name)
    }

    /// Restores a matrix as a zero-copy view into a shared packed buffer.
    pub fn restore_shared_packed_b(
        &self,
        data: Arc<Vec<u32>>,
        start: usize,
        len: usize,
        ...
        prepack::restore_shared_packed_b(
            &*self.kernel,
            data,
            start,
            len,
