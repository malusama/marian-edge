...
pub struct PackingBuffer {
    buf: Vec<PackElem>,
    shared: Option<(Arc<Vec<PackElem>>, usize, usize)>,
    used_len: usize,
}
...
impl PackingBuffer {
    ...
    pub(crate) fn from_vec(buf: Vec<PackElem>) -> Self {
        let used_len = buf.len() * size_of::<PackElem>();
        Self {
            buf,
            shared: None,
            used_len,
    ...
    pub(crate) fn from_shared(buf: Arc<Vec<PackElem>>, start: usize, len: usize) -> Option<Self> {
        ...
        Some(Self {
            buf: Vec::new(),
            shared: Some((buf, start, end)),
            used_len: len * size_of::<PackElem>(),
        })
    }
    ...
    pub fn as_bytes(&self) -> &[u8] {
        let words = match &self.shared {
            Some((buf, start, end)) => &buf[*start..*end],
            None => &self.buf,
        };
        &cast_pod_slice(words).unwrap()[..self.used_len]
    }

    /// Extract the buffer from self.
    pub fn into_vec(self) -> Vec<PackElem> {
        match self.shared {
            Some((buf, start, end)) => buf[start..end].to_vec(),
            None => self.buf,
        }
    }
}
