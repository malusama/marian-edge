...
pub enum GemmError {
    ...
    /// The serialized packed buffer length does not match its matrix shape.
    PackedDataSizeMismatch,
}
...
impl Display for GemmError {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::PackedDataSizeMismatch => {
                write!(fmt, "packed matrix buffer has the wrong size")
            }
        }
    }
}
