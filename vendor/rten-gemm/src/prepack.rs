...
pub(super) fn restore_packed_b<LhsT, RhsT, OutT>(
    kernel: &dyn Kernel<LhsT, RhsT, OutT>,
    data: Vec<u32>,
    rows: usize,
    cols: usize,
    packed_kernel_name: &str,
) -> Result<PackedBMatrix<RhsT>, GemmError> {
    if packed_kernel_name != kernel.name() {
        return Err(GemmError::PackedDataKernelMismatch);
    }
    let depth_block = depth_block_size::<RhsT>(rows);
    let layout = kernel.packed_b_layout(depth_block, cols, None);
    let tail_layout = if rows % depth_block != 0 {
        Some(kernel.packed_b_layout(rows % depth_block, cols, None))
    } else {
        None
    };
    let n_blocks = rows / depth_block;
    let expected_size =
        (n_blocks * layout.size()) + tail_layout.as_ref().map(|l| l.size()).unwrap_or(0);
    let actual_size = data
        .len()
        .checked_mul(std::mem::size_of::<u32>())
        .ok_or(GemmError::PackedDataSizeMismatch)?;
    if actual_size != expected_size {
        return Err(GemmError::PackedDataSizeMismatch);
    }
    Ok(PackedBMatrix {
        base: PackedMatrixBase {
            data: PackingBuffer::from_vec(data),
            depth_size: rows,
            nm_size: cols,
            panel_size: kernel.nr(),
            depth_block,
...
pub(super) fn restore_shared_packed_b<LhsT, RhsT, OutT>(
    kernel: &dyn Kernel<LhsT, RhsT, OutT>,
    data: Arc<Vec<u32>>,
    start: usize,
    len: usize,
    ...
    let expected_size =
        (n_blocks * layout.size()) + tail_layout.as_ref().map(|l| l.size()).unwrap_or(0);
    if len.checked_mul(std::mem::size_of::<u32>()) != Some(expected_size) {
        return Err(GemmError::PackedDataSizeMismatch);
    }
    let data =
        PackingBuffer::from_shared(data, start, len).ok_or(GemmError::PackedDataSizeMismatch)?;
    Ok(PackedBMatrix {
        base: PackedMatrixBase {
            data,
            depth_size: rows,
