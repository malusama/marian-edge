...
impl Translator {
    ...
    pub async fn translate(
        ...
        if self.inner.worker_stopped.load(Ordering::Acquire) || self.inner.tx.is_disconnected() {
        ...
        let send = self
            .inner
            .tx
            .send_async(Command::Translate(Job { input, reply }));
        match tokio::time::timeout(self.inner.enqueue_timeout, send).await {
            Ok(Ok(())) => {
    ...
    pub async fn shutdown(&self) {
        ...
        let send = self.inner.tx.send_async(Command::Shutdown(done));
        if matches!(
        ...
        let command = deferred.pop_front().or_else(|| {
            loop {
                match rx.recv_timeout(Duration::from_millis(100)) {
                    ...
                    Err(flume::RecvTimeoutError::Timeout) if !backend.is_ready() => break None,
                    Err(flume::RecvTimeoutError::Timeout) => {}
                    Err(flume::RecvTimeoutError::Disconnected) => break None,
                }
            }
        });
...
mod tests {
    ...
    async fn bounded_queue_rejects_overload_without_unbounded_waiting() {
        ...
        while translator.stats().snapshot().queue_depth < 1 {
            tokio::task::yield_now().await;
        }
